...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    y_predict = softmax(outputVectors.dot(predicted))
    cost = - np.log(y_predict[target])
    y_minus = y_predict
    y_minus[target] -= 1 #^y - y
    gradPred = (outputVectors.T).dot(y_minus)
    grad = np.outer(y_minus, predicted)
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    for contextWord in contextWords:
        cost0, gradPred0, grad0 = word2vecCostAndGradient(
            inputVectors[tokens[contextWord]],
            tokens[contextWord],
            outputVectors,
            dataset
        )
        cost += cost0
        gradIn[tokens[contextWord], :] += gradPred0
        gradOut += grad0
    #raise NotImplementedError
    ### END YOUR CODE
